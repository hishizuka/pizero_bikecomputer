...
class MenuWidget(QtWidgets.QWidget):
  ...
  def setup_ui(self):
    ...
    self.page_name_label.setStyleSheet(self.config.gui.style.G_GUI_PYQT_menu_topbar_page_name_label)
    
    self.next_button = QtWidgets.QPushButton()
    self.next_button.setEnabled(False)
    self.next_button.setFixedSize(self.icon_x, self.icon_y)
    self.next_button.setStyleSheet(self.config.gui.style.G_GUI_PYQT_menu_topbar_next_button)
  ...
  class MenuButton(QtWidgets.QPushButton):
    ...
    def setup_icon(self):
      ...
      self.right_icon.setAttribute(self.config.gui.gui_config.WA_TranslucentBackground)
      self.right_icon.setAttribute(self.config.gui.gui_config.WA_TransparentForMouseEvents)
      
      self.icon_layout = QtWidgets.QHBoxLayout(self)
      self.icon_layout.addWidget(self.right_icon, alignment=self.config.gui.gui_config.align_right)
      
    def set_icon_with_size(self):
      self.setup_icon()
      self.icon_layout.setContentsMargins(0, 0, self.icon_margin[self.button_type], 0)
      self.right_icon.setPixmap(self.icon_img[self.button_type].pixmap(self.icon_qsize[self.button_type]))
    ...
    def focusInEvent(self, event):
      ...
      if self.button_type == 'submenu' or \
        ...
        self.right_icon.setPixmap(self.icon_img['null'].pixmap(self.icon_qsize['null']))
    ...
    def change_icon_with_hover(self, status):
      self.right_icon.setPixmap(self.hover_img[self.button_type][status].pixmap(self.icon_qsize[self.button_type]))
    ...
    def change_toggle(self, status=False):
      ...
      if self.status:
        ...
        mode = 'off_hover'
      self.right_icon.setPixmap(self.toggle_img[mode].pixmap(self.icon_qsize[self.button_type]))
    ...
    def loading_stop(self, res):
      ...
      self.right_icon.setPixmap(self.res_img[res].pixmap(self.icon_qsize[self.button_type]))
      self.status = False
    ...
    def on_frameChanged(self, frameNumber):
      self.right_icon.setPixmap(QtGui.QIcon(self.loading_movie.currentPixmap()).pixmap(self.icon_qsize[self.button_type]))
...
class ListWidget(MenuWidget):
  ...
  def setup_menu(self):
    ...
    self.list.setHorizontalScrollBarPolicy(self.config.gui.gui_config.scrollbar_alwaysoff)
    self.list.setVerticalScrollBarPolicy(self.config.gui.gui_config.scrollbar_alwaysoff)
...
class ListItemWidget(QtWidgets.QWidget):
  ...
  def resizeEvent(self, event):
    h = self.size().height()
    for text, fsize in zip([self.title_label, self.detail_label], [int(h*0.45), int(h*0.4)]):
      q = text.font()
  ...
  def set_simple_list_stylesheet(self, hide_detail_label=False):
    ...
    if hide_detail_label:
      ...
      title_style = title_style + " " + self.config.gui.style.G_GUI_PYQT_menu_list_border
    else:
      detail_style = detail_style + " " + self.config.gui.style.G_GUI_PYQT_menu_list_border
...
class UploadActivityMenuWidget(MenuWidget):
  ...
  def setup_menu(self):
    ...
    button_conf = (
      #Name(page_name), button_attribute, connected functions, layout
      ('Strava', 'cloud_upload', self.strava_upload, "./img/strava_logo.svg", QtCore.QSize(self.logo_size*4, self.logo_size)),
      ('Garmin', 'cloud_upload', self.garmin_upload, "./img/garmin_logo.svg", QtCore.QSize(self.logo_size*5, self.logo_size)), 
      ('Ride with GPS', 'cloud_upload', self.rwgps_upload, "./img/rwgps_logo.svg", QtCore.QSize(self.logo_size*4, self.logo_size)), 
    )
    self.add_buttons(button_conf)
