...
class CoursesMenuWidget(MenuWidget):
  ...
  def setup_menu(self):
    ...
    button_conf = (
      ...
      ('Local Strage', 'submenu', self.load_local_course),
    ...
    if not self.config.G_GOOGLE_DIRECTION_API["HAVE_API_TOKEN"]:
      self.button['Google Directions API mode'].setEnabled(False)
      self.button['Google Directions API mode'].setProperty("style", "unavailable")
  ...
  async def load_local_course(self):
    await self.load_course('Local Strage')
  ...
  async def load_course(self, course_type):
    self.change_page(self.child_page_name, preprocess=True, reset=True, list_type=course_type)
  ...
  def onoff_course_cancel_button(self):
    if len(self.config.logger.course.distance) == 0:
      self.button['Cancel Course'].setEnabled(False)
      self.button['Cancel Course'].setProperty("style", "unavailable")
    else:
      self.button['Cancel Course'].setEnabled(True)
      self.button['Cancel Course'].setProperty("style", None)
  ...
  def cancel_course(self):
    ...
    self.config.logger.reset_course(delete_course=True)
    self.onoff_course_cancel_button()
...
class CourseListWidget(ListWidget):
  ...
  async def change_course_detail_page(self):
    if self.selected_item == None:
      return
    self.change_page(self.child_page_name, preprocess=True, course_info=self.selected_item.list_info)
  ...
  def set_course(self, course_file=None):
    ...
    if len(self.config.logger.course.distance) > 0:
      self.config.gui.show_dialog(self.cancel_and_set_new_course, 'Replace this course?')
    else:
      self.config.gui.show_dialog(self.set_new_course, 'Set this course?')

  def cancel_and_set_new_course(self):
    #cancel
    self.parentWidget().widget(self.config.gui.gui_config.G_GUI_INDEX[self.back_index_key]).cancel_course()
    #set_new_course
    self.set_new_course()
...
class CourseListItemWidget(ListItemWidget):
  ...
  def __init__ (self, parent, config, list_type=None):
    ...
    if self.list_type == "Local Strage":
      ...
      self.locality_text = ", {elevation_gain:.0f}m up, {locality}, {administrative_area}"
      if self.config.G_LANG in ["JA",]:
        self.locality_text = ", {elevation_gain:.0f}m up, {administrative_area}{locality}"      
  ...
  def add_extra(self):
    ...
    self.right_icon.setPixmap(self.parentWidget().MenuButton.icon_img['submenu'].pixmap(right_icon_qsize))
    self.right_icon.setStyleSheet(self.config.gui.style.G_GUI_PYQT_menu_list_border)

    #outer layout (custom)
    self.outer_layout.setContentsMargins(0,0,self.parentWidget().MenuButton.icon_margin['submenu']+1,0)
  ...
  def set_info(self, **kargs):
    ...
    if self.list_type == "Ride with GPS":
      self.detail_label.setText(("{:.1f}km"+self.locality_text).format(
...
class CourseDetailWidget(MenuWidget):
  ...
  def enable_next_button(self):
    ...
    self.next_button.setIcon(QtGui.QIcon(self.icon_dir+'img/forward_white.svg'))
    self.next_button.setIconSize(QtCore.QSize(20, 20))
    self.next_button.setProperty("style", "menu")
    self.next_button.setStyleSheet(self.config.gui.style.G_GUI_PYQT_buttonStyle_navi)
  ...
  async def update_display(self):
    ...
    #1st download check
    if self.privacy_code == None and self.config.network.api.check_ridewithgps_files(self.list_id, "1st"):
      self.draw_images(draw_map_image=True, draw_profile_image=False)
      self.privacy_code = self.config.logger.course.get_ridewithgps_privacycode(self.list_id)
      if self.privacy_code != None:
        #download files with privacy code (2nd download)
        await self.config.network.api.get_ridewithgps_files_with_privacy_code(self.list_id, self.privacy_code)
    #2nd download with privacy_code check
    elif self.privacy_code != None and self.config.network.api.check_ridewithgps_files(self.list_id, "2nd"):
  ...
  def check_all_image_and_draw(self):
    ...
    if not self.all_downloaded and self.config.network != None:
        self.all_downloaded = self.config.network.api.check_ridewithgps_files(self.list_id, "ALL")
  ...
  def set_course(self):
    ...
    self.parentWidget().widget(index).set_course((self.config.G_RIDEWITHGPS_API["URL_ROUTE_DOWNLOAD_DIR"]+"course-{route_id}.tcx").format(route_id=self.list_id))
  ...
  def draw_images(self, draw_map_image=True, draw_profile_image=True):
    ...
    if draw_map_image:
      filename = (self.config.G_RIDEWITHGPS_API["URL_ROUTE_DOWNLOAD_DIR"]+"preview-{route_id}.png").format(route_id=self.list_id)
      ...
      self.map_image_qsize = QtCore.QSize(int(self.map_image_size.width()*scale), int(self.map_image_size.height()*scale))
      self.map_image.setPixmap(QtGui.QIcon(filename).pixmap(self.map_image_qsize))

    if draw_profile_image:
      filename = (self.config.G_RIDEWITHGPS_API["URL_ROUTE_DOWNLOAD_DIR"]+"elevation_profile-{route_id}.jpg").format(route_id=self.list_id)
      ...
      self.profile_image_qsize = QtCore.QSize(int(self.profile_image_size.width()*scale), int(self.profile_image_size.height()*scale))
      self.profile_image.setPixmap(QtGui.QIcon(filename).pixmap(self.profile_image_qsize))
    return True
...
class GoogleDirectionsAPISettingMenuWidget(ListWidget):
  ...
  async def button_func_extra(self):
    self.config.G_GOOGLE_DIRECTION_API["API_MODE_SETTING"] = self.selected_item.title_label.text()
