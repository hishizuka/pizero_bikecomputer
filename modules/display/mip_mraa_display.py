...
# GPIO.BCM
GPIO_DISP = 13 #27 # 13 in GPIO.BOARD
GPIO_SCS = 16 #23  # 16 in GPIO.BOARD
GPIO_VCOMSEL = 11 #17  # 11 in GPIO.BOARD
...
class MipMraaDisplay(Display):
    ...
    def __init__(self, config, size=None):
        ...
        self.spi = mraa.Spi(1)
        self.spi.mode(0)
        self.spi.frequency(config.G_DISPLAY_PARAM["SPI_CLOCK"])

        self.gpio = {}
        for key in [GPIO_DISP, GPIO_SCS, GPIO_VCOMSEL]:
            self.gpio[key] = mraa.Gpio(key)
            self.gpio[key].dir(mraa.DIR_OUT)
        
        self.gpio[GPIO_SCS].write(0)
        self.gpio[GPIO_DISP].write(1)
        self.gpio[GPIO_VCOMSEL].write(1)
    ...
    def clear(self):
        self.gpio[GPIO_SCS].write(1)
        time.sleep(0.000006)
        self.spi.write(bytearray([0b00100000, 0]))  # ALL CLEAR MODE
        self.gpio[GPIO_SCS].write(0)
        time.sleep(0.000006)
    ...
    def no_update(self):
        self.gpio[GPIO_SCS].write(1)
        time.sleep(0.000006)
        self.spi.write(bytearray([0b00000000, 0]))  # NO UPDATE MODE
        self.gpio[GPIO_SCS].write(0)
        time.sleep(0.000006)
    ...
    def blink(self, sec):
        ...
        while s > 0:
            ...
            time.sleep(0.000006)
            if state:
                self.spi.write(bytearray([0b00010000, 0]))  # BLINK(BLACK) MODE
            else:
                self.spi.write(bytearray([0b00011000, 0]))  # BLINK(WHITE) MODE
            self.gpio[GPIO_SCS].write(0)
    ...
    def inversion(self, sec):
        ...
        while s > 0:
            ...
            time.sleep(0.000006)
            if state:
                self.spi.write(bytearray([0b00010100, 0]))  # INVERSION MODE
            else:
                self.no_update()
            self.gpio[GPIO_SCS].write(0)
            time.sleep(self.interval)
    ...
    async def draw_worker(self):
        while True:
            ...
            self.spi.write(bytearray(img_bytes))
            # dummy output for ghost line
            self.spi.write(bytearray([0x00000000, 0]))
            await asyncio.sleep(0.000006)
            self.gpio[GPIO_SCS].write(0)
    ...
    def update(self, im_array, direct_update):
        ...
        if direct_update and self.config.G_DISPLAY in ("MIP_Mraa_640",):
            direct_update = False
        ...
        if direct_update:
            self.gpio[GPIO_SCS].write(1)
            time.sleep(0.000006)
            self.spi.write(bytearray(self.img_buff_rgb8[diff_lines].tobytes()))
            time.sleep(0.000006)
            self.gpio[GPIO_SCS].write(0)
        # put queue
