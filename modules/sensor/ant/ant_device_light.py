...
class ANT_Device_Light(ant_device.ANT_Device):
  ...
  def setup_channel_extra(self):
    ...
    self.send_thread = threading.Thread(target=self.send_worker, name="send_worker", args=())
  ...
  def close_extra(self):
    if self.ant_state in ['quit', 'disconnect_ant_sensor']:
      ...
      time.sleep(self.ant_config['interval'][self.config.G_ANT['INTERVAL']]/32672+0.5)
  ...
  def init_after_connect(self):
    if self.values['pre_lgt_state'] == None and self.values['lgt_state'] == None and \
      self.ant_state in ['connect_ant_sensor']:
  ...
  def on_data(self, data):
    if data[0] == 0x01:
      ...
      if self.values['lgt_state'] != None and seq_no != self.page_34_count and (datetime.datetime.now() - self.values['last_changed_timestamp']).total_seconds() > self.light_retry_timeout:
        self.page_34_count -= 1
        print("Retry to change light mode...", mode, self.values['lgt_state'], seq_no, self.page_34_count, (datetime.datetime.now() - self.values['last_changed_timestamp']).total_seconds())
  ...
  def send_light_setting(self, mode):
    ...
    self.send_queue.put(
      array.array('B',[0x22,0x01,0x28,self.page_34_count,0x5A,0x10,self.light_mode_bontrager_flare_rt[mode][1],0x00])
    )
    self.values['last_changed_timestamp'] = datetime.datetime.now()
  ...
  def check_mode(self):
    if self.values['lgt_state'] != None and self.values['lgt_state'] != self.values['pre_lgt_state']:
      print("ANT+ Light mode change: {} -> {}".format(self.values['pre_lgt_state'], self.values['lgt_state']))
  ...
  def send_light_setting_light_off(self, auto=False):
    if not auto and self.values['auto_state'] and self.values['lgt_state'] != "OFF" and self.values['button_state']:
      self.values['button_state'] = False
    ...
    if self.values['lgt_state'] != "OFF" and (not auto or (auto and not self.values['button_state'])):
