...
class SensorGPS(Sensor):
    ...
    async def update_GB(self, message):
        ...
        if "time" in message:
            timestamp = (
                datetime.datetime.fromtimestamp(int(message["time"] / 1000))
                - self.gb_timediff_ftom_utc
            )
        if "hdop" in message:
    ...
    async def get_GPS_basic_values(self, lat, lon, alt, speed, track, mode, error, dop):
        ...
        if (
            lat == self.config.G_GPS_NULLVALUE
            or lon == self.config.G_GPS_NULLVALUE
            or abs(lat) > 90
            or abs(lon) > 180
            or self.values["mode"] < 3
            or np.any(
                np.isnan(
                    [self.values["pdop"], self.values["hdop"], self.values["vdop"]]
                )
            )
            or np.any(
                np.array(
        ...
        if error is not None:
            if np.any(
                ...
                np.array([self.values["epx"], self.values["epy"], self.values["epv"]])
                >= self.valid_cutoff_ep
            ):
                valid_pos = False
            # special condition #1
            if (
                self.values["used_sats"]
                < self.config.G_GPSD_PARAM["SP1_USED_SATS_CUTOFF"]
                and self.values["epv"] > self.config.G_GPSD_PARAM["SP1_EPV_CUTOFF"]
            ):
                valid_pos = False
        ...
        # track
        if (
            track != self.config.G_GPS_NULLVALUE
            and speed != self.config.G_GPS_NULLVALUE
    ...
    def set_time(self):
        ...
        if len(kernel_date_str) >= 34:
            ...
            if m:
                time_str = "{:s} {:s} {:s} 00:00:00 UTC".format(
    ...
    def get_course_index(self):
        ...
        forward_search_index_next = max(
            self.get_index_with_distance_cutoff(start, self.config.G_GPS_SEARCH_RANGE),
        ...
        for i, s in enumerate(search_indexes):
            ...
            dist_diff_mod = np.where(
                ((0 <= azimuth_diff) & (azimuth_diff <= self.azimuth_cutoff[0]))
                | ((self.azimuth_cutoff[1] <= azimuth_diff) & (azimuth_diff <= 360)),
                dist_diff,
                np.inf,
            )
            # print(dist_diff_mod)
            ...
            if np.isnan(azimuth_diff[m]):
                ...
                continue
            if (
                0 <= azimuth_diff[m] <= self.azimuth_cutoff[0]
                or self.azimuth_cutoff[1] <= azimuth_diff[m] <= 360
            ):
                # go forward
                pass
            else:
            ...
            if (
                self.course_index_check[-1] == False
                and np.sum(self.course_index_check) != 0
            ):
    ...
    def get_index_with_distance_cutoff(self, start, search_range):
        ...
        if search_range > 0:
            min_index = (
                start
                + np.abs(
                    (self.config.logger.course.distance[start:] - dist_to)
                ).argmin()
            )
        elif search_range < 0:
            min_index = np.abs(
                (self.config.logger.course.distance[0:start] - dist_to)
            ).argmin()
