...
class MMC5983MA(i2c.i2c):
  ...
  def read_mag(self):
    ...
    mag_raw = list(map(lambda x: x << 2, self.VALUE_PATTERN.unpack(bytearray(data))))
    self.values['mag'][0] = (((mag_raw[0] | (((mag_xyz[0] & 0xC0) >> 6) & 0x3)) -131072)/16384)*100
    self.values['mag'][1] = (((mag_raw[1] | (((mag_xyz[0] & 0x30) >> 4) & 0x3)) -131072)/16384)*100
    self.values['mag'][2] = self.z_sign*(((mag_raw[2] | (((mag_xyz[0] & 0x03) >> 2) & 0x3)) -131072)/16384)*100
