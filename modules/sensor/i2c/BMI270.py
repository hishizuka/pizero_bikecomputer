...
class BMI270(i2c.i2c):
    ...
    def init_sensor(self):
        ...
        # Power On Reset (POR)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, PWR_CONF, 0x00)
        time.sleep(0.00045)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, INIT_CTRL, 0x00)
        for i in range(256):
            self.bus.write_byte_data(self.SENSOR_ADDRESS, INIT_ADDR_0, 0x00)
            self.bus.write_byte_data(self.SENSOR_ADDRESS, INIT_ADDR_1, i)
            self.bus.write_i2c_block_data(
                self.SENSOR_ADDRESS,
                INIT_DATA,
                BMI270_CONFIG_FILE[i*32:(i+1)*32]
            )
            time.sleep(0.000020)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, INIT_CTRL, 0x01)
        time.sleep(0.02)

        #if self.bus.read_byte_data(self.SENSOR_ADDRESS, INTERNAL_STATUS) != 0x01:
        #    print(f"BMI270 init error: status={self.bus.read_byte_data(self.SENSOR_ADDRESS, INTERNAL_STATUS)}")

        # set mode
        # normal & performance: 0b1110 (temp=1/acc=1/gyro=1/aux=0)
        # lowpower: 0b0100 (temp=0/acc=1/gyro=0/aux=0)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, PWR_CTRL, 0b00001110)
        ...
        # power conf
        # normal, performance: 0b010 (fast power up=0/fifo self wake up=1/adv power save=0)
        # low_power: 0b011 (fast power up=0/fifo self wake up=1/adv power save=1)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, PWR_CONF, 0b10)
        
        # if lowpower mode, then VALUE_BYTES = 6

        # set range
        self.bus.write_byte_data(self.SENSOR_ADDRESS, ACC_RANGE, self.acc_range)
        self.bus.write_byte_data(self.SENSOR_ADDRESS, GYR_RANGE, self.gyro_range)

        # FIFO config
        #  fifo_gyro_en: 1 (ON)
        #  fifo_acc_en: 1 (ON)
        #  fifo_aux_en: 0 (OFF)
        #  fifo_header_en: 0 (OFF)
        #  fifo_tag_int2_en: 00
        #  fifo_tag_int1_en: 00
        self.bus.write_byte_data(
            self.SENSOR_ADDRESS,
    ...
    def read(self):
        ...
        # acc: normalized (1 is equal to G)
        self.values["acc"] = list(
            map(
                lambda x: x * self.acc_factor,
                list(self.struct_pattern.unpack(data[0:6])),
            )
        )
        # gyro: deg/sec
        self.values["gyro"] = list(
            map(
                lambda x: x * self.gyro_factor,
                list(self.struct_pattern.unpack(data[6:12])),
            )
        )
