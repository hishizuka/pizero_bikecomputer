...
class Setting():
  ...
  def read_config(self):
    ...
    if 'GENERAL' in self.config_parser:
      ...
      if 'WHEEL_CIRCUMFERENCE' in self.config_parser['GENERAL']:
        self.config.G_WHEEL_CIRCUMFERENCE = int(self.config_parser['GENERAL']['WHEEL_CIRCUMFERENCE'])/1000
      if 'GROSS_AVE_SPEED' in self.config_parser['GENERAL']:
        self.config.G_GROSS_AVE_SPEED = int(self.config_parser['GENERAL']['GROSS_AVE_SPEED'])
      if 'DISPLAY' in self.config_parser['GENERAL']:
        self.config.G_DISPLAY = self.config_parser['GENERAL']['DISPLAY'] #store temporary
      if 'AUTO_BACKLIGHT_CUTOFF' in self.config_parser['GENERAL']:
        self.config.G_AUTO_BACKLIGHT_CUTOFF = int(self.config_parser['GENERAL']['AUTO_BACKLIGHT_CUTOFF']) #store temporary
    ...
    if 'POWER' in self.config_parser:
      ...
      if 'W_PRIME' in self.config_parser['POWER']:
        self.config.G_POWER_W_PRIME = int(self.config_parser['POWER']['W_PRIME'])

    if 'ANT' in self.config_parser:
      for key in self.config_parser['ANT']:

        if key.upper() == 'STATUS':
          self.config.G_ANT['STATUS'] = self.config_parser['ANT'].getboolean(key)
          continue
        i = key.rfind("_")
        ...
        try:
          ...
          continue
        if k1 == 'USE' and k2 in self.config.G_ANT['ID'].keys(): #['HR','SPD','CDC','PWR']:
          try:
            self.config.G_ANT[k1][k2] = self.config_parser['ANT'].getboolean(key)
          except:
            pass
        elif k1 in ['ID','TYPE'] and k2 in self.config.G_ANT['ID'].keys(): #['HR','SPD','CDC','PWR']:
          try:
            self.config.G_ANT[k1][k2] = self.config_parser['ANT'].getint(key)
          except:
            pass
      for key in self.config.G_ANT['ID'].keys(): #['HR','SPD','CDC','PWR']:
        if not (0 <= self.config.G_ANT['ID'][key] <= 0xFFFF) or\
           not self.config.G_ANT['TYPE'][key] in self.config.G_ANT['TYPES'][key]:
          self.config.G_ANT['USE'][key] = False
          self.config.G_ANT['ID'][key] = 0
          self.config.G_ANT['TYPE'][key] = 0
        if self.config.G_ANT['ID'][key] != 0 and self.config.G_ANT['TYPE'][key] != 0:
          self.config.G_ANT['ID_TYPE'][key] = \
            struct.pack('<HB', self.config.G_ANT['ID'][key], self.config.G_ANT['TYPE'][key]) 
    
    if 'SENSOR_IMU' in self.config_parser:
      for s, c, m in [
        ['AXIS_CONVERSION_STATUS', 'AXIS_CONVERSION_COEF', self.config.G_IMU_AXIS_CONVERSION], 
        ['AXIS_SWAP_XY_STATUS', '', self.config.G_IMU_AXIS_SWAP_XY],
        ['MAG_AXIS_CONVERSION_STATUS', 'MAG_AXIS_CONVERSION_COEF', self.config.G_IMU_MAG_AXIS_CONVERSION], 
  ...
  def write_config(self):
    ...
    self.config_parser['GENERAL']['AUTOSTOP_CUTOFF'] = str(int(self.config.G_AUTOSTOP_CUTOFF*3.6))
    self.config_parser['GENERAL']['WHEEL_CIRCUMFERENCE'] = str(int(self.config.G_WHEEL_CIRCUMFERENCE*1000))
    self.config_parser['GENERAL']['GROSS_AVE_SPEED'] = str(int(self.config.G_GROSS_AVE_SPEED))
    self.config_parser['GENERAL']['AUTO_BACKLIGHT_CUTOFF'] = str(int(self.config.G_AUTO_BACKLIGHT_CUTOFF))
    ...
    if not self.config.G_DUMMY_OUTPUT:
      ...
      for key1 in ['USE','ID','TYPE']:
        for key2 in self.config.G_ANT[key1]:
          if key2 in self.config.G_ANT['ID'].keys(): #['HR','SPD','CDC','PWR']:
            self.config_parser['ANT'][key1+"_"+key2] = str(self.config.G_ANT[key1][key2])
    
    self.config_parser['SENSOR_IMU'] = {}
    self.config_parser['SENSOR_IMU']['AXIS_SWAP_XY_STATUS'] = str(self.config.G_IMU_AXIS_SWAP_XY['STATUS'])
    self.config_parser['SENSOR_IMU']['AXIS_CONVERSION_STATUS'] = str(self.config.G_IMU_AXIS_CONVERSION['STATUS'])
    self.config_parser['SENSOR_IMU']['AXIS_CONVERSION_COEF'] = str(list(self.config.G_IMU_AXIS_CONVERSION['COEF']))
    self.config_parser['SENSOR_IMU']['MAG_AXIS_SWAP_XY_STATUS'] = str(self.config.G_IMU_MAG_AXIS_SWAP_XY['STATUS'])
    self.config_parser['SENSOR_IMU']['MAG_AXIS_CONVERSION_STATUS'] = str(self.config.G_IMU_MAG_AXIS_CONVERSION['STATUS'])
