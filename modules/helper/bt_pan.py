...
class BTPanDbusNext(BTPan):
  ...
  async def find_bt_pan_devices(self):
    ...
    proxy_object = self.bus.get_proxy_object(self.obj_bluez, "/", await self.bus.introspect(self.obj_bluez, "/"))
    ...
    for path, ifaces in objs.items():
      if self.obj_service in ifaces.keys():
        proxy_object = self.bus.get_proxy_object(self.obj_bluez, path, await self.bus.introspect(self.obj_bluez, path))
  ...
  async def initialize_device(self, remote_addr):
    try:
      if self.remote_addr != remote_addr:
        ...
        proxy_object = self.bus.get_proxy_object(self.obj_bluez, obj_remote_addr, await self.bus.introspect(self.obj_bluez, obj_remote_addr))
...
class BTPanDbus(BTPan):
  ...
  def get_managed_objects(self):
    manager = dbus.Interface(self.bus.get_object(self.obj_bluez, "/"), self.obj_object_manager)
  ...
  def initialize_device(self, remote_addr):
    try:
      if self.remote_addr != remote_addr:
        self.interface = dbus.Interface(self.bus.get_object(self.obj_bluez, self.devices[remote_addr]), self.obj_service)
        self.remote_addr = remote_addr
    except:
      import traceback
      print(traceback.print_exc())
